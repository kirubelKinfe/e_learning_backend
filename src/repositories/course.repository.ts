import { Module, Course, Lecture, Question, Quiz, Resource, User } from '../models';
...
class CourseRepository {
    ...
    async GetCourseByDepartment(category: string): Promise<any>  {
        try{
    ...
    async CreateCourse(newCourse: CreateCourseInterface): Promise<any>  {
        ...
        try{
            ...
            await User.updateOne({ _id: instructorId },
                {
                    $set: {
    ...
    async DeleteCourse(_id: string): Promise<any>  {
        try{
            ...
            await Module.deleteMany({ courseId: _id })
            await Lecture.deleteMany({ courseId: _id })
            await Resource.deleteMany({ courseId: _id })
            await Quiz.deleteMany({ courseId: _id })
            await Question.deleteMany({ courseId: _id })
